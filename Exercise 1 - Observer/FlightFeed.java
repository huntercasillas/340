...
public class FlightFeed extends Subject {
	...
	public void start() {
		...
		if (allFlights != null && allFlights.states.size() > 0) {
			...
			FlightStatusObs status = new FlightStatusObs();
			attach(status);
			FlightDeltaObs delta = new FlightDeltaObs();
			delta.setPrevFlight(allFlights.states.get(0));
			attach(delta);
			System.out.println("Flight Info - " + _flight.toString() + "\n");
